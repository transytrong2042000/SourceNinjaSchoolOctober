...
public class ClanBattle extends GBattle {
    public static final int MAX_GTC = 30;
    ...
    public void setXu(@NotNull IGlobalBattler tocTruong, int xu) {
        ...
        if (n1.xu < xu || n2.xu < xu) {
            n1.notifyMessage("Bạn hoặc đối phương không đủ xu để cược");
            n2.notifyMessage("Bạn hoặc đối phương không đủ xu để cược");
            setState(END_STATE);
            return;
        }
...
//        }
    ...
    public boolean enter(IGlobalBattler member, int mapId) {
        ...
        if (MAX_GTC <= this.hacgia.size() && MAX_GTC <= this.bachgia.size() && !hacgia.contains(member) && !bachgia.contains(member)) {
            ninja.p.sendYellowMessage("Gia tộc chiến đã đủ người vui lòng thử lại sau");
            return false;
        }
