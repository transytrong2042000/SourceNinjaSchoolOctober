...
public class ClanTerritory implements Serializable {
    ...
    private final long WIN_WAIT_DURATION = 60_000L;
    @NotNull
    ...
    public void setState(State state) {
        ...
        if (state == State.WAITING) {
            ...
            for (ClanMember member : clanManager.members) {
                if (member.getNinja() != null && member.getNinja().p.getClanTerritoryData() != null &&
                        member.getNinja().p.getClanTerritoryData().entered) {
                    member.getNinja().pointTinhTu += 100;
                    member.getNinja().p.sendYellowMessage("Bạn nhận được 100 điểm ấn tộc");
                    if (util.nextInt(100) < 50) {  // tỉ lệ rơi đá linh hồn
                        Item it = ItemData.itemDefault(1001);
                        it.quantity = 5;
                        it.isLock = true ;
                        member.getNinja().addItemBag(true, it);
                    }
                }
            }
