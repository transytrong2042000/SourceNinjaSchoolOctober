...
public class Mob {
    ...
    public void updateHP(final int num) {
        ...
        if (this.hp <= 0) {
            ...
            if (this.isboss) {
                if (this.templates.id == 225) {
                    this.isRefresh = true;
                    this.timeRefresh = System.currentTimeMillis() + 60000L;
                } else if (this.templates.id != 198 && this.templates.id != 199 && this.templates.id != 200) {
...
//    }
    ...
    public void update(final @NotNull Place place) {
        // Skip bu nhin
        if (isThieuDot && masterThieuDot != null) {
            final Effect effId = masterThieuDot.getEffId(THIEU_DOT_ID);
            // Leave place or not exist in that map
            if (!masterThieuDot.isInPlaceAndMap(this.placeId, this.mapId) || (Math.abs(this.level -  masterThieuDot.getLevel()) >= 10)) {
                this.isThieuDot = false;
                this.masterThieuDot = null;
                return;
            }
            ...
            if (effId == null || System.currentTimeMillis() >= effId.timeRemove) {
                ...
                Service.sendThieuDot(place.getUsers(), this.id);
                if (this.templates.id == 0) return;

                val dame = effId.param * this.masterThieuDot.dameMax() / 200;
                this.updateHP(dame);
                val expUp = (long) this.level * dame;
                if (masterThieuDot instanceof Ninja) {
                    ((Ninja) masterThieuDot).p.updateExp(expUp / 240, true);
                } else {
                    ((CloneChar) masterThieuDot).chuThan.p.updateExp(expUp / 480, true);
                }
                place.attackAMob(masterThieuDot, this, dame);
            }
        }
