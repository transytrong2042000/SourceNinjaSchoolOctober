...
import static patch.Constants.PK_DOSAT;
...
public class Body implements ISoloer {
    ...
    public short caiTrang(User p ,short idItem){
        return ItemDataId(p.nj.ItemCaiTrang[0].id).part;
    }
    
    public short partHead1() {        
    ...
    public short partHead() {
        if(this.c.ItemBodyHide[0] == null){
            return this.c.head;
        }
        if (this.c.ItemBodyHide[0].id == 711){
            return 223;
        }
        if (this.c.ItemBodyHide[0].id == 714){
            return 226;
        }
        if (this.c.ItemBodyHide[0].id == 774){
            return 267;
        }
        if(this.c.gender == 0){
        if (this.c.ItemBodyHide[0].id == 786){
            return 273;
        }
        if (this.c.ItemBodyHide[0].id == 787){
            return 279;
        }
        }
        if(this.c.gender == 1){
    ...
    public void updatePk(final int num) {
        ...
        if (this.c.isTaskDanhVong == 1 && this.c.taskDanhVong[0] == 5) {
        this.c.taskDanhVong[1]++;
        if(this.c.taskDanhVong[1] == this.c.taskDanhVong[2]) {
        this.c.p.sendYellowMessage("Bạn đã hoàn thành nhiệm vụ danh vọng.");
        }
        }
        }
        if (this.pk > 20) {
    ...
    public int getMaxHP() {
        ...
        hpmax = hpmax * (100 + this.getPramItem(128)) / 100;
    ...
    public int getPramItem(final int id) {
        ...
        if(this.c.ItemBodyHide[0] != null){
        for (final Item item : this.c.ItemBodyHide) {
    ...
    public boolean fullNgoc(byte capdo) {
        byte demNgoc = 0;
        ...
        for (byte i = 0; i < 10; ++i) {
            ...
            for (byte j = 0; j < item.ngocs.size(); j++) {
                 if(item.ngocs.get(j).getUpgrade() >= capdo) {
                    demNgoc++;
                    break;
                 }
            }
    ...
    public boolean fullNgoc2(int capdo) {
        int dem2 = 0;
        int dem3 = 0;
        int dem4 = 0;
        for (byte j = 0; j < this.ItemBody.length; j++) {
            ...
            if (item != null) {
                for (int s = 0; s < item.option.size(); s++) {
                    if (item.option.get(s).id == -57 || item.option.get(s).id == -42 || item.option.get(s).id == -27 || item.option.get(s).id == -12) {
                        int up = item.option.get(s).param;
                        if (up == 10) {
                            dem4++;
                            dem3++;
                            dem2++;
    ...
    public void damage(final @Nullable Ninja other) {
        ...
        if (other.ItemBody[10] != null) {
            if ((other.ItemBody[10].id == 744 || other.ItemBody[10].id == 781) && this.isHuman) {
                return;
            }
        }
//        if (other.party != null) {  //pknhom
//            for (int i2 = 0; i2 < other.getPlace().getUsers().size(); ++i2) {
//                final User p2 = other.getPlace().getUsers().get(i2);
//                if (p2.nj.party == other.party) {
//                    return;
//                }
//            }
//        }
        ...
        int damechuan = 0;
        if (GameScr.mapNotPK(other.getMapId())) {
            if (other.getTypepk() == PK_DOSAT) {
                dame = 0;
                return;
            }
        }
        ...
        if (util.nextInt(0, 5500) < miss && this.nclass != 0) {
            dame = 0;
        }
...
//        }
        ...
        if (this.getPramItem(113) > 0) {
            damechuan += this.getPramItem(113);
            //System.err.println("Sát thương chuẩn: " + this.getPramItem(113));
            other.upHP(-damechuan);
        }
...
//        }
    ...
    public boolean isInPlaceAndMap(int placeId, int mapId) {
        if (this instanceof Ninja) {
            val nj = (Ninja) this;
            return nj.getPlace().id == placeId && nj.getMapid() == mapId;
        } else if (this instanceof CloneChar) {
            val nj = (CloneChar) this;
            return nj.c.place.id == placeId && nj.c.getMapid() == mapId;
        }
        return false;
    }
}
