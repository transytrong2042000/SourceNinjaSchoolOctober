...
public class ClanManager {
    public static final int LUONG_CREATE_CLAN = 20_000;
    ...
    public void useClanItem(final @Nullable User p, final @Nullable Message m) throws IOException {
        if (p == null || m == null) return;
        ...
        if (p.nj.clan.typeclan != TOC_TRUONG) {
            p.session.sendMessageLog("Chỉ có tộc trưởng mới có thể sử dụng");
            return;
        }
        if (item.id == 281) {
    ...
    public static void createClan(final @Nullable User p, @NotNull String name) {
        ...
        } else if (util.CheckString(name, "\\d+") && p.id == 1) {
    ...
    public void inviteToClanBattle(String name) {

        if (clanBattle == null) return;

        final Ninja ninja = getMem(name).getNinja();
        if (ninja.getClanBattle() == null) {
            ninja.setClanBattle(clanBattle);
        }
        ninja.p.sendYellowMessage("Bạn đã được mời vào gia tộc chiến");
    }
