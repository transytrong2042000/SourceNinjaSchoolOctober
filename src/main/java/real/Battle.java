...
public class Battle {
    ...
    public void setState(int state) {
        ...
        if (this.state == Battle.DAT_CUOC_STATE) {
            ...
            team1.upXuMessage(-getFinalXu());
            team2.upXuMessage(-getFinalXu());
    ...
    public void setXu(long xu,final @NotNull TeamBattle team) {
        ...
        if (xu > 10000) {
            team.notifyMessage("Xu đặt tối đa 10000 xu");
            return;
        }
    ...
    public synchronized void updateWinner(@NotNull final TeamBattle team) {
        ...
        winner.upXuMessage(getFinalXu() + getFinalXu());
        for (Ninja ninja : winner.getNinjas()) {
        if (ninja.isTaskDanhVong == 1 && ninja.taskDanhVong[0] == 2) {
        ninja.taskDanhVong[1]++;
        }
        if(ninja.taskDanhVong[1] == ninja.taskDanhVong[2]) {
        ninja.p.sendYellowMessage("Bạn đã hoàn thành nhiệm vụ danh vọng.");
        }    
