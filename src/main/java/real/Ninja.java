...
public class Ninja extends Body implements TeamBattle, IGlobalBattler {
    ...
    public byte quacap60 = 0;
    public byte quacap70 = 0;
    public byte quacap80 = 0;
    public byte quacap90 = 0;
    public byte quacap100 = 0;
    public int topSK;
    public int topSK1;
    public int topSK2;
    ...
    public byte getBSTNull() {
        byte num = 0;
        for (byte i = 0; i < this.ItemBST.length; ++i) {
            if (this.ItemBST[i] == null) {
                ++num;
            }
        }
        return num;
    ...
    public byte getIndexBSTid(final int id, final boolean lock) {
        for (byte i = 0; i < this.ItemBST.length; ++i) {
            final Item item = this.ItemBST[i];
            if (item != null && item.id == id && item.isLock() == lock) {
                return i;
            }
        }
 ...
 // xóa bỏ hành trang
    ...
    public synchronized int upxu(long y) {
        final long xunew = this.xu + y;
        if (xunew > 2000000000L) {
            y = 2000000000 - this.xu;
        } else if (xunew < -2000000000L) {
           y = -2000000000 - this.xu;
        }
        
        if (y > 1) {
        LogHistory.log8("xu tang : " + this.name + "  tang " + y + " xu || So xu cu: " + this.xu + " So xu moi: " + (this.xu + y) + " xu");
        } else if (y < -1) {
            LogHistory.log8("xu giam : " + this.name + " giam " + y + " xu || So xu cu: " + this.xu + " So xu moi: " + (this.xu + y) + " xu");
        }
        
        if (y >= 10000000) {
            LogHistory.log2("xu tang : " + this.name + "  tang " + y + " xu || So xu cu: " + this.xu + " So xu moi: " + (this.xu + y) + " xu");
        } else if (y <= -10000000){
            LogHistory.log2("xu giam : " + this.name + " giam " + y + " xu || So xu cu: " + this.xu + " So xu moi: " + (this.xu + y) + " xu");
        }

        if (y >= 200000000) {
            LogHistory.log10("xu tang : " + this.name + "  tang " + y + " xu || So xu cu: " + this.xu + " So xu moi: " + (this.xu + y) + " xu");
        } else if (y <= -200000000){
            LogHistory.log10("xu giam : " + this.name + " giam " + y + " xu || So xu cu: " + this.xu + " So xu moi: " + (this.xu + y) + " xu");
        }
        ...
        SQLManager.executeQuery("SELECT * FROM `ninja` WHERE `name`LIKE'" + name + "';", (red) -> {
            try {
                if (red != null && red.first()) {
                    ...
                    nj.quacap60 = red.getByte("quacap60");
                    nj.quacap70 = red.getByte("quacap70");
                    nj.quacap80 = red.getByte("quacap80");
                    nj.quacap90 = red.getByte("quacap90");
                    nj.quacap100 = red.getByte("quacap100");
                    nj.hailoc = red.getInt("hailoc");
                    nj.topSK = red.getInt("topSK");
                    nj.topSK1 = red.getInt("topSK1");
                    nj.topSK2 = red.getInt("topSK2");
                    nj.vuixuan = red.getInt("vuixuan");
                    nj.taytn = red.getInt("taytn");
                    nj.taykn = red.getInt("taykn");
                    ...
                    nj.Tathulenh = red.getInt("Tathulenh");
    ...
    public void flush() {
        ...
        try {
            ...
            sqlSET = sqlSET + ",`quacap60`=" + quacap60 + ",`quacap70`=" + quacap70 + ",`quacap80`=" + quacap80 + ",`quacap90`=" + quacap90 + ",`quacap100`=" + quacap100;
            ...
            sqlSET = sqlSET + ",`Tathulenh`=" + this.Tathulenh + "";
            sqlSET = sqlSET + ",`topSK1`=" + this.topSK1 + "";
            sqlSET = sqlSET + ",`topSK2`=" + this.topSK2 + "";
            sqlSET = sqlSET + ",`vuixuan`=" + this.vuixuan + "";
            sqlSET = sqlSET + ",`taytn`=" + this.taytn + "";
            sqlSET = sqlSET + ",`taykn`=" + this.taykn + "";
            sqlSET = sqlSET + ", `hailoc`=" + this.hailoc + "";
            sqlSET = sqlSET + ",`kichhoatkm`=" + kichhoatkm+ "";
