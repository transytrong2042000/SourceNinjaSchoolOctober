        ...
        try {
            ...
            switch (message.getCommand()) {
                ...
                case 102: {
                    ...
                    int FEE_GOLD = 0;
                    if (price < 0) {
                        p.session.sendMessageLog("Bạn không thể đặt giá trị âm");
                        return;
                    }
                    if (p.nj.getPlace().map.template.id != 1 && p.nj.getPlace().map.template.id != 27 && p.nj.getPlace().map.template.id != 72) {
                        p.session.sendMessageLog("qua trường mới bán được đồ ở shinwa");
                        return;
                    }
                    if (item != null && p.nj.xu >= FEE && p.luong >= FEE_GOLD) {
                        ...
                        LogHistory.log5("Shinwa: " + p.nj.name + " da ban " + itemShinwa.getItem().quantity + " item " + itemShinwa.getItem().id + " voi giá: " + itemShinwa.getPrice() + " xu.");
                        LogHistory.log8("Shinwa: " + p.nj.name + " da ban " + itemShinwa.getItem().quantity + " item " + itemShinwa.getItem().id + " voi giá: " + itemShinwa.getPrice() + " xu.");
                        if (itemShinwa.getPrice() < 0) {
                            LogHistory.log10("Shinwa: " + p.nj.name + " da ban " + itemShinwa.getItem().quantity + " item " + itemShinwa.getItem().id + " voi giá: " + itemShinwa.getPrice() + " xu.");
                        }
                    } else {
...
// Khong du xu
                        ...
                        LogHistory.log5("Shinwa: " + p.nj.name + " da mua " + itemShinwa.getItem().quantity + " item " + itemShinwa.getItem().id + " voi giá: " + itemShinwa.getPrice() + " xu.");
                        LogHistory.log8("Shinwa: " + p.nj.name + " da mua " + itemShinwa.getItem().quantity + " item " + itemShinwa.getItem().id + " voi giá: " + itemShinwa.getPrice() + " xu.");
                        if (itemShinwa.getPrice() < 0) {
                            LogHistory.log10("Shinwa: " + p.nj.name + " da mua " + itemShinwa.getItem().quantity + " item " + itemShinwa.getItem().id + " voi giá: " + itemShinwa.getPrice() + " xu.");
                        }
...
//                         Thach dau
    ...
    private void teleport(User p, String str) throws IOException {
        ...
        if (map != null && (map.isLangCo() || map.isLangTruyenThuyet() || Map.isCaveMap(map.id) || map.VDMQ() || map.loiDaiMap())) {
        ...
        try {
            if(player != null && player.nj != null && player.session != null && !player.nj.isDie && m != null && m.reader().available() > 0) {
                ...
                switch (type) {
                    ...
                    case 3: {
                        GameScr.HuyBiKip(player);
                        break;
                    }
                    case 4: {
                        GameScr.NangBiKip(player);
                        break;
                    }
                    case 5: {
                        GameScr.NangBiKip1(player);
                        break;
                    }
                    case 6: {
                        GameScr.NangAn(player, player.nj.ItemBody[29], 0);
                        break;
                    }
                    case 7: {
                        GameScr.HuyGiaToc(player);
                        break;
                    }
                    default: {
                        break;
