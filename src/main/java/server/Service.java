...
public class Service {
        ...
        try {
            ...
            switch (type) {
                case 0: {
                    ...
                    w.writeByte(2);
                    w.writeByte(id);
                    val data = GameScr.loadFile("res/effauto/x" + p.session.zoomLevel + "/ImgEffAuto/" + id + ".png").toByteArray();
                ...
                case 1: {
                    ...
                    w.writeByte(3);
                    w.writeByte(id);
                    val data = GameScr.loadFile("res/effauto/x" + p.session.zoomLevel + "/DataEffAuto/" + id).toByteArray();
    ...
    public static void openMenuGiaHan(User p, int type) {
        Message m = null;
        try {
            p.menuCaiTrang = 5;
            p.openUI(4);
            ...
            p.nj.ItemGiaHan = new Item[size];
            int pos = 0;
            for (Item item : p.nj.ItemBag) {
                if (item != null)
                if (item.expires > 0) {
                    p.nj.ItemGiaHan[pos] = item;
                    pos++;
                }
            }
            m.writer().writeByte(p.nj.ItemGiaHan.length);
