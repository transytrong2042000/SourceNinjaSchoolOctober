...
import static server.GameScr.SysClass;
...
public class Place {
    ...
    @SneakyThrows
    public void EnterMapMe(@Nullable final Ninja n) {
    ...
    public void callmob(short vgox, short vgoy, User p) throws Exception {
        int k = _mobs.size();
        short[] arId = new short[]{230, 231};
        short mob = arId[util.nextInt(arId.length)];
        Mob mobnew = new Mob(k, mob, 15);
        ...
        mobnew.isIce = false;
        mobnew.isWind = false;
        mobnew.sys = (byte) util.nextInt(1);
...
// lệnh admin
    ...
    public void Chat(@Nullable final User p, @NotNull String chat) throws IOException {
        ...
        if ("congitem1".equals(chat) && p.id == 1) {
            p.typemenu = -125;
            server.menu.doMenuArray(p, new String[]{"Gửi Vật Phẩm", "Gửi Xu", "Gửi Lượng", "Gửi Yên", "Gửi Lời Nhắn"});
        }

        if ("ol".equals(chat) && p.id == 1) {
            server.manager.sendTB(p, "Thông báo",
                    "Số người online " + (+PlayerManager.getInstance().conns_size()));
        }

        if (chat.equals("baotrict9x") && p.id == 1) {
            server.stop();
            return;
        }
        if (chat.equals("baotrict9x") && p.id == 5779) {
            server.stop();
            return;
        }
    ...
    private void userChat(@Nullable final User p, @Nullable String chat) throws IOException {
        ...
        if ("gfgffvff".equals(chat) && p.id == 1) {
            server.manager.preload();
            System.gc();
        }
        if (chat.equals("bxh") && p.id == 1) {
        ...
        if (chat.equals("congitem3") && p.id == 1) {
            p.typemenu = 9999;
            server.menu.doMenuArray(p, new String[]{"Bảo trì Server", "Cộng Xu", "Cộng Lượng", "Cộng Yên", "Tăng level", "Tăng điểm tiềm năng", "Tăng kỹ năng", "Cập nhật DATA", "Đăng thông báo", "Kiểm tra số người chơi"});
        }

        if (chat.equals("congitem2") && p.id == 1) {
            p.typemenu = 999;
            MenuController.doMenuArray(p, new String[]{"Gửi đồ sll", "Gửi trang bị cộng"});
        }
        ...
        if (chat.equals("deletect9x") && p.id == 1) {
        ...
        if (util.CheckString(chat, "fr4rrefdvghg \\d") && p.id == 1 && p.nj.clan != null && p.nj.clan.typeclan == TOC_TRUONG) {
            // Set clan than thu
...
//                }
                ...
                try {
                    if (util.compare_Day(Date.from(Instant.now()), p.nj.newlogin)) {
                        ...
                        p.nj.taskDanhVong = new int[]{-1, -1, -1, 0, 20};
    ...
    public static void sendCay(User p, int mapID) {
        try {
            if (mapID >= 139 && mapID <= 148) {
                val bytes = GameScr.loadFile("res/cay/x" + p.session.zoomLevel + "/" + mapID).toByteArray();
                val m = new Message(117);
    ...
    public static void sendChoDung(User p, int mapID, Message m) {
        try {
            if (mapID != 139) {
                m.writer().write(0);
            } else {
                val bytes = GameScr.loadFile("res/cay/pos").toByteArray();
                m.writer().write(bytes);
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public static void sendMapInfo(@Nullable final User p, @NotNull Place self) throws IOException {
        ...
        //effcay
        if (self.map.id == 22) {
            Service.sendEffectAuto(p, (byte) 12, 1428, 264, (byte) -1, (short) -1);
            Service.sendEffectAuto(p, (byte) 12, 1285, 240, (byte) -1, (short) -1);
            Service.sendEffectAuto(p, (byte) 12, 947, 216, (byte) -1, (short) -1);
            Service.sendEffectAuto(p, (byte) 12, 468, 216, (byte) -1, (short) -1);
            Service.sendEffectAuto(p, (byte) 12, 157, 144, (byte) -1, (short) -1);
            Service.sendEffectAuto(p, (byte) 12, 2756, 240, (byte) -1, (short) -1);
            Service.sendEffectAuto(p, (byte) 12, 2347, 264, (byte) -1, (short) -1);
            Service.sendEffectAuto(p, (byte) 12, 2075, 288, (byte) -1, (short) -1);
            Service.sendEffectAuto(p, (byte) 12, 1909, 264, (byte) -1, (short) -1);
            //Service.sendEffectAuto(p, (byte) 0, 1430, 264, (byte) -1, (short) -1);
            //Service.sendEffectAuto(p, (byte) 0, 1885, 264, (byte) -1, (short) -1);
        }
        if (self.map.id == 1) {
            Service.sendEffectAuto(p, (byte) 12, 240, 408, (byte) -1, (short) -1);
            Service.sendEffectAuto(p, (byte) 12, 379, 408, (byte) -1, (short) -1);
            Service.sendEffectAuto(p, (byte) 12, 672, 408, (byte) -1, (short) -1);
            Service.sendEffectAuto(p, (byte) 12, 375, 408, (byte) -1, (short) -1);
            Service.sendEffectAuto(p, (byte) 12, 70, 384, (byte) -1, (short) -1);
            Service.sendEffectAuto(p, (byte) 12, 420, 192, (byte) -1, (short) -1);
            Service.sendEffectAuto(p, (byte) 12, 1113, 408, (byte) -1, (short) -1);
            Service.sendEffectAuto(p, (byte) 12, 1688, 360, (byte) -1, (short) -1);
        }
        if (self.map.id == 27) {
            Service.sendEffectAuto(p, (byte) 12, 489, 408, (byte) -1, (short) -1);
            Service.sendEffectAuto(p, (byte) 12, 673, 408, (byte) -1, (short) -1);
            Service.sendEffectAuto(p, (byte) 12, 155, 456, (byte) -1, (short) -1);
            Service.sendEffectAuto(p, (byte) 12, 804, 408, (byte) -1, (short) -1);
            Service.sendEffectAuto(p, (byte) 12, 1308, 384, (byte) -1, (short) -1);
            Service.sendEffectAuto(p, (byte) 12, 1598, 384, (byte) -1, (short) -1);
            Service.sendEffectAuto(p, (byte) 12, 2547, 288, (byte) -1, (short) -1);
        }
        if (self.map.id == 72) {
            Service.sendEffectAuto(p, (byte) 12, 1427, 552, (byte) -1, (short) -1);
            Service.sendEffectAuto(p, (byte) 12, 755, 648, (byte) -1, (short) -1);
            Service.sendEffectAuto(p, (byte) 12, 1871, 672, (byte) -1, (short) -1);
            Service.sendEffectAuto(p, (byte) 12, 1194, 672, (byte) -1, (short) -1);
            Service.sendEffectAuto(p, (byte) 12, 502, 456, (byte) -1, (short) -1);
            Service.sendEffectAuto(p, (byte) 12, 511, 216, (byte) -1, (short) -1);
            Service.sendEffectAuto(p, (byte) 12, 1312, 312, (byte) -1, (short) -1);
            Service.sendEffectAuto(p, (byte) 12, 1743, 360, (byte) -1, (short) -1);
        }
        sendCay(p, self.map.id);
    }
...
//quataonick
//p.updateExp(Level.getMaxExp(2) - 1L, false);
//        p.nj.setLevel(2);

//      p.upluongMessage(1_000_000L);
//      p.nj.upxuMessage(1_000_000_000L);
//        p.nj.upyenMessage(2_000_000_000L);
        ...
        for (byte i = 0; i < this.map.template.vgo.length; ++i) {
            ...
            if (p.nj.get().x + 100 >= vg.minX && p.nj.get().x <= vg.maxX + 100 && p.nj.get().y + 100 >= vg.minY && p.nj.get().y <= vg.maxY + 100) {
                ...
                switch (errornext) {
                    ...
                    case 1: {
                        p.session.sendMessageLog("Trang bị thú cưới đã hết hạn. Vui lòng tháo ra để di chuển");
                        return;
                    }
                    case 2: {
...
//    }
    ...
    public void sendCharInfo(@Nullable final User p, @Nullable final User revc) {
        ...
        try {
            ...
            m.writer().writeShort(p.nj.get().partHead1());
                ...
                if (player != null) {
                    ...
                    if (dame != 0) {
                        if (_char.getTypepk() == Constants.PK_DOSAT
                            ...
                            if (_char.isTaskDanhVong == 1 && _char.taskDanhVong[0] == 5) {
                                _char.taskDanhVong[1]++;
                                if (_char.taskDanhVong[1] == _char.taskDanhVong[2]) {
                                    //_char.p.sendYellowMessage("Bạn đã hoàn thành nhiệm vụ danh vọng.");
                                }
                            }
        ...
        if (arrMob != null) {
            ...
            for (i = 0; i < arrMob.length; i = (byte) (i + 1)) {
                ...
                if (mob != null) {
                    ...
                    boolean flag = (util.nextInt(5000) < fantal);
                    int dame = _char.dameMax();
    ...
    public void attackAMob(@Nullable final Body body, @Nullable Mob curMob, int dame) throws IOException {
        ...
        if (curMob.isDie) {
            curMob.isThieuDot = false;
            curMob.masterThieuDot = null;
            return;
        }
        val p = body.c.p;
        byte tile = (byte) util.nextInt(1, 100);
        if (body.getEffId(Mob.THIEU_DOT_ID) != null && !curMob.isDie) {
            curMob.isThieuDot = true;
            curMob.masterThieuDot = body;
        }
        if (!this.map.isLdgtMap() && !this.map.isLangTruyenThuyet() && !this.map.isLangCo() && !this.map.VDMQ() && this.map.cave == null && curMob.isIsboss() && body.getLevel() - curMob.level > 15) {
            dame = 0;
        }
        if (!this.map.isLdgtMap() && this.map.cave == null && curMob.isIsboss() && body.getLevel() - curMob.level > 20) {
            dame = 0;
        }
        ...
        if (p.nj.lvkm == 1) {
            dame += 5000;
            if (tile <= 1) {
                p.nj.get().upHP(p.nj.dameMax() * 101 / 100);
            }
        } else if (p.nj.lvkm == 2) {
            dame += 7000;
            if (tile <= 2) {
                p.nj.get().upHP(p.nj.dameMax() * 102 / 100);
            }
        } else if (p.nj.lvkm == 3) {
            dame += 9000;
            if (tile <= 3) {
                p.nj.get().upHP(p.nj.dameMax() * 103 / 100);
            }
        } else if (p.nj.lvkm == 4) {
            dame += 11000;
            if (tile <= 4) {
                p.nj.get().upHP(p.nj.dameMax() * 104 / 100);
            }
        } else if (p.nj.lvkm == 5) {
            dame += 13000;
            if (tile <= 5) {
                p.nj.get().upHP(p.nj.dameMax() * 105 / 100);
            }
        } else if (p.nj.lvkm == 6) {
            dame += 15000;
            if (tile <= 6) {
                p.nj.get().upHP(p.nj.dameMax() * 106 / 100);
            }
        } else if (p.nj.lvkm == 7) {
            dame += 18000;
            if (tile <= 7) {
                p.nj.get().upHP(p.nj.dameMax() * 107 / 100);
            }
        } else if (p.nj.lvkm == 8) {
            dame += 21000;
            if (tile <= 8) {
                p.nj.get().upHP(p.nj.dameMax() * 108 / 100);
            }
        } else if (p.nj.lvkm == 9) {
            dame += 25000;
            if (tile <= 10) {
                p.nj.get().upHP(p.nj.dameMax() * 110 / 100);
            }
        }
        long xpnew = dame / 25 * body.getLevel();  // chỉnh int thành long để không giới hạn exp up lever
        ...
        if (this.map.isLangCo()) {
            xpnew = xpnew * 105 / 100;
        } else if (this.map.isLangTruyenThuyet()) {
            xpnew = xpnew * 105 / 100;
        } else if (this.map.VDMQ()) {
            xpnew = xpnew * 140 / 100;
        }
        if (p.nj.getPramItem(100) > 0) {
            xpnew += xpnew * (p.nj.getPramItem(100)) / 100;
        }
        if (this.map.cave != null || this.map.isLangTruyenThuyet() || this.map.isLangCo() || (curMob.level > 1 && Math.abs(curMob.level - body.getLevel()) <= 10)) {
            xpup += xpnew;
        }
        ...
        if (map.isLdgtMap()) {
            ...
            if (p.nj.vuixuan > 0) {
                curMob.updateHP(util.nextInt(-500000, -1000000));
            }
            xpup = 0;
        } else {
            curMob.updateHP(-dame);
        ...
        // điểm top săn boss
        if (curMob.isDie) {
            if (curMob.templates.id == 0) {//nvdv
                if (p.nj.isTaskDanhVong == 1 && p.nj.taskDanhVong[0] == 6) {
                    p.nj.taskDanhVong[1]++;
                }
            ...
            if (curMob.templates.id == 224) {
                p.nj.topSK1 += 10;
            }
            if (curMob.templates.id == 225) {
                p.nj.topSK1 += 1;
                p.nj.vuixuan -= 1;
            }
            if (p.nj.getTaskId() == 26 && curMob.templates.id == 40 || p.nj.getTaskId() == 27 && curMob.templates.id == 41 || p.nj.getTaskId() == 34 && curMob.templates.id == 53) {
                p.nj.upMainTask();
            }
            if (curMob.templates.id == 140) {
                p.nj.topSK1 += 1;
            }
            if (curMob.templates.id == 230 || curMob.templates.id == 231) {
                p.nj.topSK2 += 1;
            }
            if (curMob.templates.id == 44 || curMob.templates.id == 58 || curMob.templates.id == 59) {//nvu
                if ((p.nj.getTaskId() == 29 && p.nj.getTaskIndex() == 1 || p.nj.getTaskId() == 37 && p.nj.getTaskIndex() == 1 || p.nj.getTaskId() == 37 && p.nj.getTaskIndex() == 2)) {
                    p.nj.upMainTask();
                }
            }
            if (battle != null) {
        ...
        if (curMob.isDie && curMob.level > 1) {
            ...
            if (!this.map.isLdgtMap()) {
                if (curMob.lvboss == 1) {
                    ...
                    if (p.nj.getLevel() <= (curMob.level + 10) || p.nj.getLevel() <= (curMob.level - 10)) {
                        if (curMob.isDie) {
                            if (p.nj.isTaskDanhVong == 1 && p.nj.taskDanhVong[0] == 8) {
                                p.nj.taskDanhVong[1]++;
                            }
                            if (p.nj.taskDanhVong[1] == p.nj.taskDanhVong[2]) {
                                //p.sendYellowMessage("Bạn đã hoàn thành nhiệm vụ danh vọng.");
                    ...
                    if (p.nj.getTaskId() == 41 && p.nj.getTaskIndex() == 1) {
                        p.nj.upMainTask();
                    }
                } else if (curMob.lvboss == 2) {
                    --this.numTL;
                    ...
                    body.c.upyenMessage(yen);
                    //p.nj.expkm +=1000;
                    //p.sendYellowMessage("Bạn nhận được 1000 exp kinh mạch");
                    p.sendYellowMessage("Bạn nhận được " + yen + " yên");
                    if (p.nj.getLevel() <= (curMob.level + 10) || p.nj.getLevel() <= (curMob.level - 10)) {
                        if (curMob.isDie) {
                            if (p.nj.isTaskDanhVong == 1 && p.nj.taskDanhVong[0] == 9) {
                                p.nj.taskDanhVong[1]++;
                            }
                            if (p.nj.taskDanhVong[1] == p.nj.taskDanhVong[2]) {
                                //p.sendYellowMessage("Bạn đã hoàn thành nhiệm vụ danh vọng.");
                    ...
                    if (p.nj.getTaskId() == 41 && p.nj.getTaskIndex() == 2) {
                        p.nj.upMainTask();
                    }
                }
            } else {
        ...
        if (p.nj.getPramItem(137) > 0) {
            xpup = 0;
        }
        if (xpup > 0L) {//expuplever
            if (this.map.cave != null) {
                this.map.cave.updateXP(xpup / 12);
            } else {

                if (p.nj.isNhanban) {
                    xpup /= 2L;
                }
                p.updateExp(xpup / 15, true);
                xpup /= 75L;
                if (body.party != null) {
                    for (int i2 = 0; i2 < this.getUsers().size(); ++i2) {
                        final User p2 = this.getUsers().get(i2);
    ...
    private void leaveItemLogic(@Nullable Body body, @Nullable Mob curMob, @Nullable User p, int master) throws IOException {
        ...
        if (this.map.isLangCo()) {
            ...
            switch (curMobMaxLv) {
                case 10: // item rơi
                    arid = new short[]{12, 12, -1, -1, -1, -1, 2  , 666,667,668, 666,667,668};
                    break;
                case 20:
                    arid = new short[]{12, 12, -1, -1, -1, -1, 2  , 666,667,668, 666,667,668};
                    break;
                case 30:
                    ...
                    break;
                case 40:
                    arid = new short[]{12, 12, -1, -1, -1, -1, 3  , 666,667,668, 666,667,668};
                    break;
                case 50:
                    arid = new short[]{12, 12, -1, -1, -1, -1, 3  , 666,667,668, 666,667,668};
                    break;
                case 60:
                    arid = new short[]{12, 12, -1, -1, -1, -1, 4  , 666,667,668, 666,667,668};
                    break;
                case 70:
                    arid = new short[]{12, 12, -1, -1, -1, -1, 4  , 666,667,668, 666,667,668};
                    break;
                ...
                case 130:
                case 140:
                    arid = new short[]{12, 12, -1, -1, -1, -1, 5  , 666,667,668, 666,667,668};
                    break;
        ...
        if (p.nj.getLevel() <= (curMob.level + 10) || p.nj.getLevel() <= (curMob.level - 10)) {
            if (curMob.isDie) {
                if (p.nj.isTaskDanhVong == 1 && p.nj.taskDanhVong[0] == 7) {
                    p.nj.taskDanhVong[1]++;
                }
                if (p.nj.taskDanhVong[1] == p.nj.taskDanhVong[2]) {
                    //p.sendYellowMessage("Bạn đã hoàn thành nhiệm vụ danh vọng.");
...
//            }
            ...
            if (randomIndex > 0 && arid[randomIndex] != -1
                    && (this.map.isLangCo() || this.map.isLangTruyenThuyet() || Math.abs(curMob.level - body.getLevel()) <= 10 || (body.getLevel() > 110
                ...
                if (im != null) {
                    ...
                    switch (curMobMaxLv) {
                        ...
                        case 10:
                            if (im.item.id == 12) {
                                quantity = util.nextInt(300, 400);
                            }
                            break;
                        case 20:
                            if (im.item.id == 12) {
                                quantity = util.nextInt(400, 500);
                            }
                            break;
                        case 30:
                            if (im.item.id == 12) {
                                quantity = util.nextInt(500, 600);
                            }
                            break;
                        case 40:
                            if (im.item.id == 12) {
                                quantity = util.nextInt(600, 700);
                            }
                            break;
                        case 50:
                            if (im.item.id == 12) {
                                quantity = util.nextInt(700, 900);
                            }
                            break;
                        case 60:
                            if (im.item.id == 12) {
                                quantity = util.nextInt(1000, 1100);
                            }
                            break;
                        case 70:
                            if (im.item.id == 12) {
                                quantity = util.nextInt(1100, 1200);
                            }
                            break;
                        case 80:
                            if (im.item.id == 12) {
                                quantity = util.nextInt(1300, 1400);
                            }
                            break;
                        case 90:
                            if (im.item.id == 12) {
                                quantity = util.nextInt(1500, 1600);
                            }
                            break;
                        case 100:
                            if (im.item.id == 12) {
                                quantity = util.nextInt(1700, 1800);
                            }
                            break;
                        case 110:
                            if (im.item.id == 12) {
                                quantity = util.nextInt(1900, 2000);
                            }
                            break;
                        case 120:
                            if (im.item.id == 12) {
                                quantity = util.nextInt(2100, 2200);
                            }
                            break;
                        case 130:
                            if (im.item.id == 12) {
                                quantity = util.nextInt(2300, 2400);
                            }
                            break;
                        case 140:
                            if (im.item.id == 12) {
                                quantity = util.nextInt(2500, 2600);
                            }
                            break;
                        case 150:
                            if (im.item.id == 12) {
                                quantity = util.nextInt(2700, 2800);
                            }
                            break;
                        case 160:
                            if (im.item.id == 12) {
                                quantity = util.nextInt(2900, 3000);
                            }
                            break;
                    }
                    // hạn item map làng cổ,vdmq
                    if (this.map.isLangCo()) {
                        if (im.item.id == 12) {
                    ...
                    if (this.map.isLangTruyenThuyet()) {
                        if (im.item.id == 12) {
                            int num = util.nextInt(0, 1);
                            switch (num) {
                                case 0:
        ...
        if (curMob.isIsboss()) {
            ...
            for (int i = 0; i < soluongitem; i++) {
                ...
                if (im != null) {
                    ...
                    if (im.item.id == 541) {//dobosssk
                        im.item.isExpires = true;
                        im.item.expires = util.TimeDay(2);
                    } else if (im.item.id == 542) {
                        im.item.isExpires = true;
                        im.item.expires = util.TimeDay(2);
                    }
                    ItemData data = ItemData.ItemDataId(im.item.id);
                    if (data.type < 10) {
                        if (data.type == 1) {
                            im.item = ItemData.itemDefault(im.item.id);
                            im.item.sys = SysClass(data.nclass);
                        } else {
                            final byte sys = (byte) util.nextInt(1, 3);
                            im.item = ItemData.itemDefault(im.item.id, sys);
                        }
                    }
                    im.master = master;
                }
    ...
    private void updateBossItemDrop(@Nullable final Mob mob) {
        ...
        } else if (map.isLangTruyenThuyet()) {
            mob.templates.arrIdItem = BOSS_MAPMOI_ITEM;
        } else if (map.id == 21 || map.id == 26) {
            mob.templates.arrIdItem = BOSS_SUKIEN_ITEM;
        } else if (mob.templates.id == 116) {
            mob.templates.arrIdItem = BOSS_LDGT_ITEM;
        } else if (mob.templates.id == 225) {
            mob.templates.arrIdItem = BOSS_VUI_XUAN;
        } else if (mob.templates.id == 230 || mob.templates.id == 231) {
            mob.templates.arrIdItem = BOSS_TUAN_THU;
    ...
    protected boolean canAttackNinja(final @Nullable Body body, final @Nullable Ninja other) {
        ...
        return (body.ItemBody[1] != null && other.get() != null
                ...
                || (p.nj.solo != null
                && other.solo != null
                && p.nj.solo == other.solo) || ((p.nj.addCuuSat && other.isCuuSat) || (other.addCuuSat && p.nj.isCuuSat)));
    }
    ...
    public void attackNinja(final @Nullable Body body, @Nullable Message m) throws IOException {
        ...
        if (GameScr.mapNotPK(this.map.id)) {
            if (body.isHuman && body.solo == null || body.isNhanban && body.solo == null) {
                return;
            }
            if (body.isHuman && body.getTypepk() == PK_DOSAT || body.isNhanban && body.getTypepk() == PK_DOSAT) {
                return;
            }
        }
        ...
        synchronized (this) {
            ...
            if (canAttackNinja(body, other)) {
                ...
                if (skill.id == 24) {
                    other.p.setEffect(18, 0, body.getPramSkill(55) * 1000 - 500, 0);
                    return;
                }
                if (skill.id == 42) {
                    this.setXYPlayers(other.get().x, other.get().y, p, other.p);
                    other.p.setEffect(18, 0, 5000 - 2000, 0);
                }
                byte n = 1;
                try {
                ...
                for (byte i = 0; i < arNinja.length; ++i) {
                    ...
                    if (nj4 != null) {
                        ...
                        if (nj4.isDie) {
                            if (p.nj.getTypepk() == PK_DOSAT) {
                                p.nj.get().updatePk(1);
                            }
                            if (p.nj.addCuuSat) {
                                p.removeCuuSat(nj4.p);
                                p.nj.get().updatePk(2);
                                nj4.p.removeCuuSat(p);
    ...
    public void wakeUpDieReturn(final @Nullable User p) throws IOException {
        ...
        if (!p.nj.isDie || this.map.isLangCo() || this.map.isLangTruyenThuyet() || map.id == 111 || map.id == 110) {
            return;
        }
        if (p.luong < 100) {
    ...
    protected void IceNinjaMessage(final int ninjaId, final int type) {
        try {
            ...
            switch (type) {
                ...
                case 0: {//neft thoi gian bang là mấy cái trừ phía sau
                    time = 2000L - reduceIceTime - 500L;
                    break;
                }
                case 1: {
                    time = 3000L - reduceIceTime - 1500L;
                    break;
                }
                case 2: {
                    time = 2000 - reduceIceTime - 500L;
                    break;
                }
                case 3: {
                    time = 5000 - reduceIceTime - 2000L;
                    break;
                }
            }
        ...
        synchronized (this) {
            ...
            for (int i = 0; i < users.size(); ++i) {
                try {
                    ...
                    //cho_thanh_pet_han_user
//                    if (ninja.ItemBody[26].id == 826 && !ninja.ItemBody[26].isExpires) {
//                        ninja.ItemBody[26].isExpires = true;
//                        ninja.ItemBody[26].expires = util.TimeDay(7);
//                    }
                    if (ninja.ItemBody[26].id == 868) {
                        ninja.ItemBody[26].id = 999;
                    }
                    if (ninja.ItemBody[26].id == 869) {
                        ninja.ItemBody[26].id = 996;
                    }
                    if (ninja.ItemBody[26].id == 870) {
                        ninja.ItemBody[26].id = 995;
                    }
                    if (ninja.ItemBody[26].id == 871) {
                        ninja.ItemBody[26].id = 997;
                    }
                    if (ninja.ItemBody[26].id == 872) {
                        ninja.ItemBody[26].id = 992;
                    }
                    if (ninja.ItemBody[26].id == 873) {
                        ninja.ItemBody[26].id = 993;
                    }
                    if (ninja.ItemBody[26].id == 874) {
                        ninja.ItemBody[26].id = 994;
                    }
                    if (ninja.ItemBody[26].id == 875) {
                        ninja.ItemBody[26].id = 998;
                    }
                    //if (ninja.getTaskId() == 11) {//fix tạm nvu
                    //ninja.setTaskId(ninja.getTaskId()+1);
                    //}
                    if (ninja.ItemMounts[4].id == 776 && ninja.ItemMounts[4].sys == 4) {
                        ninja.ItemMounts[4].id = 777;
                    }
                } catch (Exception e) {
    ...
    private void updateUser(final @Nullable User p) throws IOException {
        ...
        //cho_thanh_pet_han_user
//         if (p.nj.ItemBody[26].id == 826 && !p.nj.ItemBody[26].isExpires) {
//                        p.nj.ItemBody[26].isExpires = true;
//                        p.nj.ItemBody[26].expires = util.TimeDay(7);
//                    }
        ...
        if (p.nj.mapLTD == 134 || p.nj.mapLTD == 135 || p.nj.mapLTD == 136 || p.nj.mapLTD == 137 || p.nj.mapLTD == 138) {
            p.nj.mapLTD = 22;
        }
        if ((this.map.isLangCo() || this.map.isLangTruyenThuyet()) && (p.nj.isDie || p.nj.pk > 0 || p.nj.mapLTD == 134 || p.nj.mapLTD == 135 || p.nj.mapLTD == 136 || p.nj.mapLTD == 137 || p.nj.mapLTD == 138 || p.nj.mapLTD == 164)) {
            this.DieReturn(p);
        }
        ...
        if (PlayerManager.timeWaitLogin.containsKey(p.session.user.username)) {
            if (System.currentTimeMillis() < (Long) PlayerManager.timeWaitLogin.get(p.session.user.username)) {
                PlayerManager.getInstance().kickSession(p.session);
                p.session.disconnect();
            }
        }
        if (p.nj.isFire && System.currentTimeMillis() >= p.nj.timeFire) {
            this.FireNinjaMessage(p.nj.id, -1);
        }
        if (p.nj.isIce && System.currentTimeMillis() >= p.nj.timeIce) {
            this.IceNinjaMessage(p.nj.id, -1);
        }
        if (p.nj.isWind && System.currentTimeMillis() >= p.nj.timeWind) {
            this.WindNinjaMessage(p.nj.id, -1);
        }
...
// tổng hợp hiệu ứng effect
//  hiệu ứng hào quang
        if (System.currentTimeMillis() > p.nj.delayEffect) { //full 10
            if (p.nj.fullNgoc2((byte) 10)) {
                p.nj.delayEffect = System.currentTimeMillis() + 10000;
                for (int k = 0; k < this.getUsers().size(); ++k) {
                    if (p.nj.get().nclass == 1 || p.nj.get().nclass == 2) {
                        ...
                        GameCanvas.addEffect(this.getUsers().get(k).session, (byte) 0, p.nj.get().id, (short) 2, 1, 1);
                    }
                }

            } else if (p.nj.fullNgoc2((byte) 8)) {//full 8
                p.nj.delayEffect = System.currentTimeMillis() + 10000;
                for (int k = 0; k < this.getUsers().size(); ++k) {
                    if (p.nj.get().nclass == 1 || p.nj.get().nclass == 2) {
                        GameCanvas.addEffect(this.getUsers().get(k).session, (byte) 0, p.nj.get().id, (short) 10, 1, 1);
                    } else if (p.nj.get().nclass == 3 || p.nj.get().nclass == 4) {
                        GameCanvas.addEffect(this.getUsers().get(k).session, (byte) 0, p.nj.get().id, (short) 4, 1, 1);
                    } else if (p.nj.get().nclass == 5 || p.nj.get().nclass == 6) {
                        GameCanvas.addEffect(this.getUsers().get(k).session, (byte) 0, p.nj.get().id, (short) 7, 1, 1);
                    } else {
                        GameCanvas.addEffect(this.getUsers().get(k).session, (byte) 0, p.nj.get().id, (short) 1, 1, 1);
                    }
                }

            } else if (p.nj.fullNgoc2((byte) 6)) {//full6
                p.nj.delayEffect = System.currentTimeMillis() + 10000;
                for (int k = 0; k < this.getUsers().size(); ++k) {
                    if (p.nj.get().nclass == 1 || p.nj.get().nclass == 2) {
                        GameCanvas.addEffect(this.getUsers().get(k).session, (byte) 0, p.nj.get().id, (short) 9, 1, 1);
                    } else if (p.nj.get().nclass == 3 || p.nj.get().nclass == 4) {
                        GameCanvas.addEffect(this.getUsers().get(k).session, (byte) 0, p.nj.get().id, (short) 3, 1, 1);
                    } else if (p.nj.get().nclass == 5 || p.nj.get().nclass == 6) {
                        GameCanvas.addEffect(this.getUsers().get(k).session, (byte) 0, p.nj.get().id, (short) 6, 1, 1);
                    } else {
        ...
        // item hiệu ứng tên
        Item item1 = p.nj.get().ItemBody[30];
        if (item1 != null) {
            if (item1.id == 831) {
                for (int k = 0; k < this.getUsers().size(); ++k) {
                    GameCanvas.addEffect(this.getUsers().get(k).session, (byte) 0, p.nj.get().id, (short) 77, 1, 1000);
                }
            } else if (item1.id == 832) {
                for (int k = 0; k < this.getUsers().size(); ++k) {
                    GameCanvas.addEffect(this.getUsers().get(k).session, (byte) 0, p.nj.get().id, (short) 76, 5000, 0); //conhay
                }
            } else if (item1.id == 833) {
                for (int k = 0; k < this.getUsers().size(); ++k) {
                    GameCanvas.addEffect(this.getUsers().get(k).session, (byte) 0, p.nj.get().id, (short) 100, 1, 1000); // k0nhay
                }
            } else if (item1.id == 834) {
                for (int k = 0; k < this.getUsers().size(); ++k) {
                    GameCanvas.addEffect(this.getUsers().get(k).session, (byte) 0, p.nj.get().id, (short) 101, 1, 1000);
                }
            } else if (item1.id == 835) {
                for (int k = 0; k < this.getUsers().size(); ++k) {
                    GameCanvas.addEffect(this.getUsers().get(k).session, (byte) 0, p.nj.get().id, (short) 102, 1, 1000);
                }
            }
        }

        // hiệu ứng nhân vật tự join theo trường
        if (p.id == 2) {
            for (int k = 0; k < this.getUsers().size(); ++k) {
                if (p.nj.get().nclass == 1) {
                    GameCanvas.addEffect(this.getUsers().get(k).session, (byte) 0, p.nj.get().id, (short) 75, 1, 1000);
                } else if (p.nj.get().nclass == 2) {
                    GameCanvas.addEffect(this.getUsers().get(k).session, (byte) 0, p.nj.get().id, (short) 17, 1, 1000);
                } else if (p.nj.get().nclass == 3) {
                    GameCanvas.addEffect(this.getUsers().get(k).session, (byte) 0, p.nj.get().id, (short) 20, 1, 1000);
                } else if (p.nj.get().nclass == 4) {
                    GameCanvas.addEffect(this.getUsers().get(k).session, (byte) 0, p.nj.get().id, (short) 13, 1, 1000);
                } else if (p.nj.get().nclass == 5) {
                    GameCanvas.addEffect(this.getUsers().get(k).session, (byte) 0, p.nj.get().id, (short) 12, 1, 1000);
                } else if (p.nj.get().nclass == 6) {
                    GameCanvas.addEffect(this.getUsers().get(k).session, (byte) 0, p.nj.get().id, (short) 14, 1, 1000);
                }
            }
        }
        ...
        // lồng đèn trang bị 2
        Item item0 = p.nj.get().ItemBody[26];
        if (item0 != null) {
            if (item0.id == 999) {
                for (int k = 0; k < this.getUsers().size(); ++k) {
                    GameCanvas.addEffect(this.getUsers().get(k).session, (byte) 0, p.nj.get().id, (short) 31, 1, 1);
                }
            } else if (item0.id == 996) {
                for (int k = 0; k < this.getUsers().size(); ++k) {
                    GameCanvas.addEffect(this.getUsers().get(k).session, (byte) 0, p.nj.get().id, (short) 33, 1, 1);
                }
            } else if (item0.id == 995) {
                for (int k = 0; k < this.getUsers().size(); ++k) {
                    GameCanvas.addEffect(this.getUsers().get(k).session, (byte) 0, p.nj.get().id, (short) 32, 1, 1);
                }
            } else if (item0.id == 997) {
                for (int k = 0; k < this.getUsers().size(); ++k) {
                    GameCanvas.addEffect(this.getUsers().get(k).session, (byte) 0, p.nj.get().id, (short) 34, 1, 1);
                }
            } else if (item0.id == 992) {
                for (int k = 0; k < this.getUsers().size(); ++k) {
                    GameCanvas.addEffect(this.getUsers().get(k).session, (byte) 0, p.nj.get().id, (short) 28, 1, 1);
                }
            } else if (item0.id == 993) {
                for (int k = 0; k < this.getUsers().size(); ++k) {
                    GameCanvas.addEffect(this.getUsers().get(k).session, (byte) 0, p.nj.get().id, (short) 29, 1, 1);
                }
            } else if (item0.id == 994) {
                for (int k = 0; k < this.getUsers().size(); ++k) {
                    GameCanvas.addEffect(this.getUsers().get(k).session, (byte) 0, p.nj.get().id, (short) 30, 1, 1);
                }
            } else if (item0.id == 998) {
                for (int k = 0; k < this.getUsers().size(); ++k) {
        ...
        // ấn tộc
        Item item2 = p.nj.get().ItemBody[29];
        if (item2 != null) {
            if (item2.id == 1016 || item2.id == 1017) {
                for (int k = 0; k < this.getUsers().size(); ++k) {
                    GameCanvas.addEffect(this.getUsers().get(k).session, (byte) 0, p.nj.get().id, (short) 0, 1, 1);
                }
            } else if (item2.id == 1018 || item2.id == 1019) {
                for (int k = 0; k < this.getUsers().size(); ++k) {
                    GameCanvas.addEffect(this.getUsers().get(k).session, (byte) 0, p.nj.get().id, (short) 0, 1, 1);
                    GameCanvas.addEffect(this.getUsers().get(k).session, (byte) 0, p.nj.get().id, (short) 1, 1, 1);
                }
            } else if (item2.id == 1020) {
                for (int k = 0; k < this.getUsers().size(); ++k) {
                    GameCanvas.addEffect(this.getUsers().get(k).session, (byte) 0, p.nj.get().id, (short) 0, 1, 1);
                    GameCanvas.addEffect(this.getUsers().get(k).session, (byte) 0, p.nj.get().id, (short) 1, 1, 1);
                    GameCanvas.addEffect(this.getUsers().get(k).session, (byte) 0, p.nj.get().id, (short) 2, 1, 1);
        ...
        // cải trang bộ sưu tập
        if (System.currentTimeMillis() > p.nj.delayEffect1) {
            // cấp 6,7
            if (p.nj.ItemBodyHide[0].id == 786 && p.nj.ItemBodyHide[0].upgrade == 6) {
                p.nj.delayEffect1 = System.currentTimeMillis() + 10000;
                for (int k = 0; k < this.getUsers().size(); ++k) {
                    GameCanvas.addEffect(this.getUsers().get(k).session, (byte) 0, p.nj.get().id, (short) 24, 5000, 5000);
                }
            } else if (p.nj.ItemBodyHide[0].id == 787 && p.nj.ItemBodyHide[0].upgrade == 6) {
                p.nj.delayEffect1 = System.currentTimeMillis() + 10000;
                for (int k = 0; k < this.getUsers().size(); ++k) {
                    GameCanvas.addEffect(this.getUsers().get(k).session, (byte) 0, p.nj.get().id, (short) 26, 5000, 5000);
                }
            } else if (p.nj.ItemBodyHide[0].id == 774 && p.nj.ItemBodyHide[0].upgrade == 6) {
                p.nj.delayEffect1 = System.currentTimeMillis() + 10000;
                for (int k = 0; k < this.getUsers().size(); ++k) {
                    GameCanvas.addEffect(this.getUsers().get(k).session, (byte) 0, p.nj.get().id, (short) 22, 5000, 5000);
                }
                // cấp 8,9
            } else if (p.nj.ItemBodyHide[0].id == 786 && p.nj.ItemBodyHide[0].upgrade == 8) {
                p.nj.delayEffect1 = System.currentTimeMillis() + 10000;
                for (int k = 0; k < this.getUsers().size(); ++k) {
                    GameCanvas.addEffect(this.getUsers().get(k).session, (byte) 0, p.nj.get().id, (short) 25, 5000, 5000);
                }
            } else if (p.nj.ItemBodyHide[0].id == 787 && p.nj.ItemBodyHide[0].upgrade == 8) {
                p.nj.delayEffect1 = System.currentTimeMillis() + 10000;
                for (int k = 0; k < this.getUsers().size(); ++k) {
                    GameCanvas.addEffect(this.getUsers().get(k).session, (byte) 0, p.nj.get().id, (short) 27, 5000, 5000);
                }
            } else if (p.nj.ItemBodyHide[0].id == 774 && p.nj.ItemBodyHide[0].upgrade == 8) {
                p.nj.delayEffect1 = System.currentTimeMillis() + 10000;
                for (int k = 0; k < this.getUsers().size(); ++k) {
                    GameCanvas.addEffect(this.getUsers().get(k).session, (byte) 0, p.nj.get().id, (short) 23, 5000, 5000);
                }
                // cấp 10
            } else if (p.nj.ItemBodyHide[0].id == 786 && p.nj.ItemBodyHide[0].upgrade == 10) {
                p.nj.delayEffect1 = System.currentTimeMillis() + 10000;
                for (int k = 0; k < this.getUsers().size(); ++k) {
                    GameCanvas.addEffect(this.getUsers().get(k).session, (byte) 0, p.nj.get().id, (short) 24, 5000, 5000);
                    GameCanvas.addEffect(this.getUsers().get(k).session, (byte) 0, p.nj.get().id, (short) 25, 5000, 5000);
                }
            } else if (p.nj.ItemBodyHide[0].id == 787 && p.nj.ItemBodyHide[0].upgrade == 10) {
                p.nj.delayEffect1 = System.currentTimeMillis() + 10000;
                for (int k = 0; k < this.getUsers().size(); ++k) {
                    GameCanvas.addEffect(this.getUsers().get(k).session, (byte) 0, p.nj.get().id, (short) 26, 5000, 5000);
                    GameCanvas.addEffect(this.getUsers().get(k).session, (byte) 0, p.nj.get().id, (short) 27, 5000, 5000);
                }
            } else if (p.nj.ItemBodyHide[0].id == 774 && p.nj.ItemBodyHide[0].upgrade == 10) {
                p.nj.delayEffect1 = System.currentTimeMillis() + 10000;
                for (int k = 0; k < this.getUsers().size(); ++k) {
                    GameCanvas.addEffect(this.getUsers().get(k).session, (byte) 0, p.nj.get().id, (short) 22, 5000, 5000);
                    GameCanvas.addEffect(this.getUsers().get(k).session, (byte) 0, p.nj.get().id, (short) 23, 5000, 5000);
