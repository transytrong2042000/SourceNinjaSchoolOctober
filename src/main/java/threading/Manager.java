...
public class Manager {
    ...
    public static short[] BOSS_LDGT_ITEM;
    ...
    public void preload() {
        ...
        if (rotationluck[1] == null) {
            this.rotationluck[1] = new RotationLuck("Vòng xoay lượng", (byte) 1, (short) 120, 1000, 100_000, 500_000_000, "lượng");
    ...
    private void loadProperties() {
        ...
        try (InputStream inputStream = new FileInputStream("application.properties")) {
            ...
            MAP_MOI_ITEM_IDS = parseShortArray(properties.getProperty("map-moi-item-ids"));
            ...
            BOSS_VUI_XUAN = parseShortArray(properties.getProperty("boss-vui-xuan"));
            BOSS_TUAN_THU = parseShortArray(properties.getProperty("boss-tuan-thu"));
            BOSS_LC_ITEM = parseShortArray(properties.getProperty("boss-item-lc"));
            BOSS_MAPMOI_ITEM = parseShortArray(properties.getProperty("boss-item-mapmoi"));
            BOSS_SUKIEN_ITEM = parseShortArray(properties.getProperty("boss-item-sukien"));
            BOSS_LDGT_ITEM = parseShortArray(properties.getProperty("boss-item-ldgt"));
        ...
        try {
            ...
            SQLManager.executeQuery("SELECT * FROM `optionitem`;", (res) -> {
                ...
                while (res.next()) {
                    ...
                    iOptionTemplates[i[0]] = iotemplate;
                    ++i[0];
                }
                res.close();
            });
    ...
    private void setCache() {
        try {
            ...
            dos.writeByte(mapCache.length);
            for (short i = 0; i < mapCache.length; ++i) {
                dos.writeUTF(mapCache[i].mapName);
            }
            dos.writeByte(npcCache.length);
            for (short i = 0; i < npcCache.length; ++i) {
                dos.writeUTF(npcCache[i].name);
                dos.writeShort(npcCache[i].headId);
                dos.writeShort(npcCache[i].bodyId);
                dos.writeShort(npcCache[i].legId);
                String[][] menu = npcCache[i].menu;
                dos.writeByte(menu.length);
                for (String[] me : menu) {
                    dos.writeByte(me.length);
                    for (String s : me) {
                        dos.writeUTF(s);
                    }
                }
            }
            dos.writeByte(mobCache.length);
            for (short i = 0; i < mobCache.length; i++) {
                dos.writeByte(mobCache[i].type);
                dos.writeUTF(mobCache[i].name);
                dos.writeInt(mobCache[i].hp);
                dos.writeByte(mobCache[i].rangeMove);
                dos.writeByte(mobCache[i].speed);
            }
    ...
    public static void getPackMessage(User p) {
        Message m = null;
        try {
            m = new Message(-28);
...
//    }
    ...
    public void createCacheMap(final User p) throws IOException{
        Message m = new Message(-28);
        try {
            m.writer().writeByte(-121);
            m.writer().writeByte(this.vsMap);
            m.writer().writeByte(mapCache.length);
            for (short i = 0; i < mapCache.length; ++i) {
                m.writer().writeUTF(mapCache[i].mapName);
            }
            m.writer().writeByte(npcCache.length);
            for (short i = 0; i < npcCache.length; ++i) {
                m.writer().writeUTF(npcCache[i].name);
                m.writer().writeShort(npcCache[i].headId);
                m.writer().writeShort(npcCache[i].bodyId);
                m.writer().writeShort(npcCache[i].legId);
                String[][] menu = npcCache[i].menu;
            ...
            m.writer().writeByte(mobCache.length);
            for (short i = 0; i < mobCache.length; i++) {
                m.writer().writeByte(mobCache[i].type);
                m.writer().writeUTF(mobCache[i].name);
                m.writer().writeInt(mobCache[i].hp);
                m.writer().writeByte(mobCache[i].rangeMove);
                m.writer().writeByte(mobCache[i].speed);
            }
            m.writer().flush();
            p.sendMessage(m);
            m.cleanup();
